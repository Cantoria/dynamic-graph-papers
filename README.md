...
- [动态图表示学习、动态图分析论文汇总项目](#动态图表示学习动态图分析论文汇总项目)
  ...
  - [New Works of Dynamic Graph Representation (Updating)](#new-works-of-dynamic-graph-representation-updating)
      ...
      - [Hierarchical Temporal Convolutional Networks for Dynamic Recommender Systems](#hierarchical-temporal-convolutional-networks-for-dynamic-recommender-systems)
      ...
      - [Streaming Graph Neural Networks](#streaming-graph-neural-networks)
  - [Other Related Works](#other-related-works)
    - [Heterogeneous Graph/Heterogeneous Information Network](#heterogeneous-graphheterogeneous-information-network)
    ...
    - [Dynamic \& Heterogeneous Graph Representation](#dynamic--heterogeneous-graph-representation)
      ...
      - [基于动态异构信息网络的时序关系预测](#基于动态异构信息网络的时序关系预测)
  ...
  - [Large Language Models for Dynamic Graph (Updating)](#large-language-models-for-dynamic-graph-updating)
      - [Temporal Knowledge Graph Forecasting Without Knowledge Using In-Context Learning](#temporal-knowledge-graph-forecasting-without-knowledge-using-in-context-learning)
      - [Back to the Future: Towards Explainable Temporal Reasoning with Large Language Models](#back-to-the-future-towards-explainable-temporal-reasoning-with-large-language-models)
      - [GenTKG: Generative Forecasting on Temporal Knowledge Graph](#gentkg-generative-forecasting-on-temporal-knowledge-graph)
  - [Related Datasets](#related-datasets)
  - [其他参考资料](#其他参考资料)
    - [图神经网络相关学习/参考资料：](#图神经网络相关学习参考资料)
      - [图与机器学习课程](#图与机器学习课程)
      - [时态表示学习与推理研究论坛](#时态表示学习与推理研究论坛)
...
* 发表于：ICLR 2017
* 标签：图神经网络
* 概述：提出了图卷积神经网络（GCN）的概念，并使用其聚合、激活节点的一阶邻居特征。
...
* 发表于：ICLR 2018
* 标签：Inductive Graph Embedding
* 概述：相较于GCN/GraphSage等模型，该模型提出了图注意力机制的思想，在信息传递时计算目标节点邻居节点的权重，依据权重进行信息聚合。该模型能够适用于transductive/inductive任务。
...
* 发表时间：2020.6
* 发表于：arXiv
* 标签：动态图表示，CTDG，inductive learning
...
* 相关数据集：
    ...
    * Cora
    * DBLP
    * 作者整理的数据集合：https://drive.google.com/drive/folders/14CcDB_aEVA3RjALFUYl69utB7ErViENo
...
* 发表时间：2020
* 发表于：SIGIR 2020
* 标签：CTDG，Streaming Graph
* 概述：该论文类似于论文Real-Time Streaming Graph Embedding Through Local Actions，结合图神经网络，提出了一套面向离散时态动态图的流式处理框架DyGNN。该框架包括更新部分（update component）与扩散模块（propagation component）。更新部分包括interact/update/merge三个单元，能够依据事件(source,target,t)，分别生成事件表示、两个节点的隐藏表示与cell memory以及生成两个节点的最终表示；扩散模块包括interact/prop/merge三个模块，利用注意力机制为事件的source与target节点所对应的一跳邻居（即influenced nodes）更新Cell memory与隐藏表示。最终的节点表示取决于每个节点所维护最新的两套Cell memory与隐藏单元。作者在链接预测与节点分类上进行了实验。
* 链接：https://dl.acm.org/doi/abs/10.1145/3397271.3401092
* 相关数据集：
    * UCI
    * DNC
    * Epinions
* 是否有开源代码：有（https://github.com/alge24/dygnn ）
...
## Large Language Models for Dynamic Graph (Updating)
#### Temporal Knowledge Graph Forecasting Without Knowledge Using In-Context Learning
* 作者：Dong-Ho Lee, Kian Ahrabian, Woojeong Jin, Fred Morstatter, Jay Pujara
* 发表时间：2023
* 发表于：EMNLP
* 标签：Temporal Knowledge Graph, Large Language Models
* 概述：这篇论文研究了大模型在时序知识图谱（Temporal Knowledge Graph，TKG）预测任务上的效果。作者提出了一个框架，将相关的历史事实转化为提示，并生成排名预测。作者发现，LLMs在不进行微调或设计显式模块的情况下，能够与专门设计和训练用于TKG预测的模型取得相当的效果。
...
* 发表于：Arxiv
* 标签：Temporal Knowledge Graph, Large Language Models
* 概述：这篇论文介绍了一个可解释时序推理任务，该任务基于上下文预测在未来时间戳上事件的发生，随后为其预测提供明确的解释。提出的任务综合评估了LLMs在复杂时间推理、未来事件预测和可解释性方面的能力。为支持这一任务，文章提出了首个多源指导调整数据集，名为可解释时间推理（ExpTime），包括来自时序知识图谱数据集及其时序推理路径的26,000条数据。基于这个数据集，我们提出了首个开源的大模型TimeLlaMA。
* 链接：https://arxiv.org/abs/2310.01074
* 是否有开源代码：无

#### GenTKG: Generative Forecasting on Temporal Knowledge Graph
* 作者：Ruotong Liao, Xu Jia, Yunpu Ma, Volker Tresp
* 发表时间：2023
* 发表于：Arxiv
* 标签：Temporal Knowledge Graph, Large Language Models
* 概述：这篇文章在提出了生成式时序推理。由于复杂的时序知识图谱数据结构和LLMs能够处理的自然语言表达之间存在巨大差距，同时TKG的庞大数据规模和微调LLMs的高计算成本之间也存在矛盾。为了解决这些问题，文章提出了一种检索增强生成框架，用于在tKG上执行生成式预测，名为GenTKG，它结合了基于时间逻辑规则的检索策略和轻量级参数高效的指导调整。大量实验证明，在计算资源有限的情况下，GenTKG优于传统的时序知识预测方法。
